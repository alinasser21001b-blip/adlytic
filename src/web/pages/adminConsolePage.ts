...
//  activation/subscriptions, edit accounts, and inspect activity.
//  Data is gated by /api/admin/* + PLATFORM_ADMIN_EMAILS.
// ════════════════════════════════════════════════════════════════════════
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .tab {
      ...
      background: var(--surface); color: var(--text-2); font-weight: 700; font-size: 13px;
    }
    .tab.active { background: rgba(217,167,89,0.14); color: var(--accent-2); border-color: rgba(217,167,89,0.35); }
      ...
      <section class="panel view" id="view-customers">
        <div class="panel-head">
          <div>
            ...
            <div class="panel-sub">إنشاء · تفعيل · تعديل · نشاط</div>
          </div>
          <div class="toolbar">
...
<script>
(function () {
  var state = { customers: [], subscriptions: [], events: [], overview: null, detail: null };
  ...
  function renderSubscriptions(rows) {
    ...
    tbody.innerHTML = (rows || []).map(function (w) {
      ...
      return '<tr>'
        ...
        + '</div></td>'
        + '</tr>';
    }).join('') || '<tr><td colspan="6" class="empty">لا اشتراكات بعد.</td></tr>';
  }
  ...
  function renderLedger(events) {
    ...
    tbody.innerHTML = (events || []).map(function (e) {
      return '<tr>'
        ...
        + '</tr>';
    }).join('') || '<tr><td colspan="5" class="empty">لا أحداث بعد.</td></tr>';
  }
    ...
    try {
      ...
      var wsHtml = (d.workspaces || []).map(function (w) {
        return '<div class="list-card">'
          ...
          + '<div class="muted">الحالة: ' + esc(w.subscriptionStatus) + ' · ينتهي: ' + esc(fmtShort(w.subscriptionExpiresAt)) + '</div>'
  ...
  document.body.addEventListener('click', function (e) {
    var t = e.target.closest('[data-open],[data-activate],[data-deactivate],[data-grant],[data-cancel]');
    if (!t) return;
