...
//
//  Eight panes, in chain order:
//    1 META TRUTH → 2 SEMANTICS → 3 ANOMALIES → 4 EVIDENCE
//    → 5 DIAGNOSIS → 6 DECISION → 7 LLM LAYER → 8 TRACE
//
//  ── FRONTEND CONTRACT ─────────────────────────────────────────────────
//
...
//    RECOMMENDATION · DO_NOT_DO · LLM_EXPLANATION
// ════════════════════════════════════════════════════════════════════════
...
(function () {
  ...
  function render(d) {
    ...
    html += '<div style="margin-bottom:14px;">'
      + '<div style="font-size:17px;font-weight:700;">' + esc(d.campaign.name) + '</div>'
      + '<div class="muted">' + esc(d.campaign.id) + ' · Meta ' + esc(d.campaign.externalCampaignId)
      + ' · ' + esc(d.campaign.status) + '</div></div>';

    // 1 META TRUTH
    html += stage(1, 'META TRUTH', 'data status: ' + d.metaTruth.dataStatus,
      ...
      + kv('Data status', d.metaTruth.dataStatus, d.metaTruth.dataStatus === 'COMPLETE' ? 'pill-ok' : 'pill-warn')
      + '</div>' + factTable(d.metaTruth.facts));

    // 2 SEMANTICS
    html += stage(2, 'SEMANTICS', 'purpose → primary KPI → result unit',
      '<div class="kv" style="margin-bottom:12px;">'
    ...
    html += stage(3, 'ANOMALIES', d.anomalies.significant ? 'SIGNIFICANT' : 'not significant',
      '<div class="kv" style="margin-bottom:12px;">'
    ...
    html += stage(4, 'EVIDENCE', d.evidence.count + ' issue row(s)', evHtml);

    // 5 DIAGNOSIS
    html += stage(5, 'DIAGNOSIS', 'decided by ' + d.diagnosis.decidedBy,
      '<div class="kv" style="margin-bottom:12px;">'
    ...
    var auditRows = d.decision.actionAudit.map(function (a) {
      return '<tr>'
        + '<td><span class="kind kind-' + (a.permitted ? 'RECOMMENDATION' : 'DO_NOT_DO') + '">'
          + (a.permitted ? 'PERMITTED' : 'BLOCKED') + '</span></td>'
        + '<td>' + esc(a.actionCode) + '</td>'
        + '<td colspan="3" class="src">' + (a.reason ? esc(a.reason) : '') + '</td>'
        + '</tr>';
    }).join('');
    html += stage(6, 'DECISION', 'guarded by permitAction()',
      '<div class="kv" style="margin-bottom:12px;">'
    ...
    html += stage(7, 'LLM LAYER', 'authoritative: false', llmBody);

    // 8 TRACE
    var traceHtml = d.trace.length
      ? d.trace.map(function (t) {
          return '<div class="trace-step"><span class="trace-layer">' + esc(t.layer) + '</span>'
            + '<span>' + esc(t.conclusion) + '</span></div>';
        }).join('')
    ...
    html += stage(8, 'TRACE', 'reconcileIntelligence() layer record', traceHtml);
