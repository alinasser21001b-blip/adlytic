...
function buildIssueMarkerDataset(labels, isoDates, issueDates) {
  ...
  var severityColor = { CRITICAL: 'var(--critical)', HIGH: 'var(--error)', MEDIUM: 'var(--warning)', LOW: 'var(--text-3)' };
  var isoToIndex = {};
  ...
  issueDates.forEach(function (iss) {
    ...
    colors.push(severityColor[iss.severity] || 'var(--text-3)');
  });
...
var METRIC_GLOSSARY = {
  ...
  link_clicks: {
    label: 'النقرات على الرابط (Link Clicks)',
    def: 'عدد النقرات التي أدّت فعلياً لفتح الوجهة (موقع، واتساب، ماسنجر) — بخلاف "النقرات" العامة التي تشمل أيضاً التفاعل والتعليقات والمشاركات وتكبير الصور.',
    formula: 'مجموع نقرات الرابط التي فتحت الوجهة فعلياً، دون احتساب التفاعلات الأخرى.',
    range: 'يُقرأ دائماً مع تكلفة النقرة (CPC) — وهو المقام الصحيح لتكلفة الزيارة الفعلية للوجهة.',
    causes: ['نص الإعلان يشجّع التفاعل أكثر من زيارة الوجهة', 'زر الدعوة لاتخاذ إجراء غير واضح أو غائب', 'الجمهور يتفاعل مع المحتوى دون قصد زيارة الوجهة'],
  },
  messages: {
