...
`DetectedIssue` is `date`-scoped per entity (`@@index([entityType, entityId, date])`) — it is already, structurally, a timeline of "something happened on this day" events. The chart's x-axis is already dates (`chart-spend-main` in `dashboardPage.ts` builds `labels` from the same `DailyStat[]` window). Timeline Explorer is a **join**, not a new data source: for each point on the existing chart, look up whether a `DetectedIssue` exists for that entity+date, and if so, render a marker.

### 3.2 What's missing: attributing a spike to a specific cause

A `DetectedIssue` says "budget waste detected on this campaign on this day" — it does not say "because ad set X's frequency crossed 5.2." That attribution needs the same driver-decomposition logic the dashboard's `renderAttribution()` already does for a fixed date range (`attribution.drivers.{impressions,ctr,cvr}` in the existing DTO) — but currently only for the whole 30-day window, computed once. Timeline Explorer needs this computed **per spike**, on demand (when the user clicks a marker), not eagerly for every day — eager computation would multiply the existing attribution query by the number of days in the window for a feature only used when a marker is actually clicked.

### 3.3 Design

- **Markers** (chart-side, near-zero backend cost): a new lightweight endpoint, `GET /api/workspaces/:wsId/campaigns/:campaignId/issue-dates?days=30`, returns `{ date, issueCode, severity }[]` — a direct `DetectedIssue` query, no new computation. Chart.js renders these as a secondary scatter dataset pinned to the x-axis (small dots under the line, colored by severity using the existing `--warning`/`--error`/`--critical` tokens).
