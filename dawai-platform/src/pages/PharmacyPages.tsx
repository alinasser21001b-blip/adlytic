...
export function PharmacySettingsPage() {
  ...
  return (
    <>
      ...
      <form className="product-form" onSubmit={submit}><div className="settings-identity"><Icon name="store" size={25} /><div><strong>{profile.data.name}</strong><span>{profile.data.district} · {profile.data.address}</span></div><StatusBadge status={profile.data.verification_status} /></div><label className="toggle-field"><input name="accepting" type="checkbox" defaultChecked={profile.data.accepting_requests} /><span>استقبال طلبات جديدة</span></label><label className="toggle-field"><input name="paused" type="checkbox" defaultChecked={profile.data.operational_status === "PAUSED"} /><span>إيقاف الفرع مؤقتًا</span></label><label className="toggle-field"><input name="pickup" type="checkbox" defaultChecked={profile.data.pickup_enabled} /><span>الاستلام متاح</span></label><label className="toggle-field"><input name="delivery" type="checkbox" defaultChecked={profile.data.delivery_enabled} /><span>التوصيل متاح</span></label><div className="form-divider"><span>ساعات العمل</span></div><div className="form-row"><label className="form-field"><span>يفتح</span><input name="opensAt" type="time" defaultValue="08:00" required /></label><label className="form-field"><span>يغلق</span><input name="closesAt" type="time" defaultValue="23:59" required /></label></div><div className="weekday-grid">{["الأحد", "الاثنين", "الثلاثاء", "الأربعاء", "الخميس", "الجمعة", "السبت"].map((day, index) => <label className="toggle-field" key={day}><input name={`day-${index}`} type="checkbox" defaultChecked={(profile.data.opening_hours?.[String(index)] ?? []).length > 0} /><span>{day}</span></label>)}</div>{message ? <p className="inline-notice" role="status">{message}</p> : null}<button className="primary-cta" type="submit">حفظ الإعدادات</button></form>
    </>
  );
}
