...
### Matrix row template — to be filled by a real run

| capability | endpoint | version | object | level | field/breakdown | permission | **token result** | account dep. | freshness | API cost | analytical value | tier |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| _(empty until the probe runs)_ | | | | | | | | | | | | |

Tiers: **S** strategic · **A** advanced · **B** diagnostic · **C** secondary ·
**D** not worth the complexity. Tier is assigned **after** the token result,
never before — a Tier-S idea that this token cannot read is Tier D in practice.
