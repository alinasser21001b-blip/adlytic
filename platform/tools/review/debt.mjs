...
export const DEBT = [
  {
    id: "TD-1",
    description: "The patient app has no HOST. The composition root now exists — `main/createRuntime` assembles the real ports, cache, outbox flusher, telemetry and authority from a `Host` interface, and a test drives a search end to end through it — so the application has been assembled and started. What is missing is a platform under it: nothing supplies a Host (no React Native project, no browser bundle), so nothing MOUNTS it on a device or in a page. Separately, the CLIENT half of the infrastructure exists: one HTTP transport classifying through @dawai/net, IdentityPort and CataloguePort over the declared contracts, the outbox flusher driving retry/backoff/idempotency, and an effect runner closing the store → server → store loop — all proved against a scripted fake server. What does not exist is a server: no backend has ever answered a real request, no session token is stored or refreshed, and deviceId/baseUrl have no production source.",
    impact: "The composed app now starts and searches under test. What remains unexercised is everything a real platform brings: no host means no device or browser has ever run it, and no backend means real latency, pagination, token refresh, TLS and server-side authorization remain unexercised until a backend exists, and nothing has been observed on a device.",
    priority: "critical",
